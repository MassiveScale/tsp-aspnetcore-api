...
- `clean-output-dirs` option (default `true`). Before emitting, previously-generated files are removed from every output directory (models, interfaces, controllers, services, helpers, and validators), so renamed or deleted TypeSpec types no longer leave stale `.g.cs` files behind. Cleaning is guarded to never delete hand-written code: a file is removed only when it both matches `file-extension` **and** carries the `<auto-generated>` marker written by the default `file` template. An extension-matching file without the marker is left in place and reported with the new `output-file-left-behind` warning; files with any other extension are ignored. Directories are scanned recursively. Set `clean-output-dirs: false` to disable. See [Output cleaning](docs/output-cleaning.md).
